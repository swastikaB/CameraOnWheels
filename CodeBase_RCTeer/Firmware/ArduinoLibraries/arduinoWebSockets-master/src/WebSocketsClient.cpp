...
bool WebSocketsClient::sendTEXT(uint8_t * payload, size_t length, bool headerToPayload) {
    if(length == 0) {
        length = strlen((const char *) payload);
    }
    if(clientIsConnected(&_client)) {
        return sendFrame(&_client, WSop_text, payload, length, true, false, headerToPayload);
    }
    return false;
}

bool WebSocketsClient::sendTEXT(const uint8_t * payload, size_t length) {
    return sendTEXT((uint8_t *) payload, length);
}

bool WebSocketsClient::sendTEXT(char * payload, size_t length, bool headerToPayload) {
    return sendTEXT((uint8_t *) payload, length, headerToPayload);
}

bool WebSocketsClient::sendTEXT(const char * payload, size_t length) {
    return sendTEXT((uint8_t *) payload, length);
}

bool WebSocketsClient::sendTEXT(String & payload) {
    return sendTEXT((uint8_t *) payload.c_str(), payload.length());
}
...
bool WebSocketsClient::sendBINARY(uint8_t * payload, size_t length, bool headerToPayload) {
    if(clientIsConnected(&_client)) {
        return sendFrame(&_client, WSop_binary, payload, length, true, false, headerToPayload);
    }
    return false;
}

bool WebSocketsClient::sendBINARY(const uint8_t * payload, size_t length) {
    return sendBINARY((uint8_t *) payload, length);
}
